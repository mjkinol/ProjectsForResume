...
public:
	...
	/* returns a vector of all words that would be formed by executing the
           given move. The first element of the pair is a string containing
            the word
	   ...
	   This function does not check that the words formed are actually in 
	   the dictionary.
	   The words returned from this function must be checked against the 
	   dictionary to
	...
	/* Executes the given move by taking tiles and placing them on the board.
	   ...
	   i.e., the corresponding square has that letter (with score 0) 
	   placed on it.
	*/
	...
	std::pair<std::string, unsigned int> getMainWord(const PlaceMove &m, std::vector<Tile*> wordTiles) const;
...
private:
	...
	unsigned int cols;
	bool firstMove;
	std::vector <std::vector<Square*>> gameSquare;
