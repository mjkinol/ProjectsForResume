...
public:
	...
	/* Parses the given move m, and constructs the corresponding move subclass.
	   m is assumed to be in the format described on the assignment sheet for 
	   a move command.
...
public:
	...
	//Add more public/protected/private functions/variables here.
private:
 	std::string ExmoveTiles;
};

// represents a place move, where a player places 
// one or more tiles onto the board.
class PlaceMove : public Move
{

public:
	/* Creates a PLACE move, starting at row y, column x, placing the tiles
	   described by the string tileString. If "horizontal" is true, then
	   the tiles are placed horizontally, otherwise vertically.
	   Coordinates start with 1.
	...
	*/
	...
	std::string getMoveTiles() const;
	//Add more public/protected/private functions/variables here.
private:
	size_t x1; //cols
	size_t y1; //rows
	bool hzntl;
	std::string moveTiles;
